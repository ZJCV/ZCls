...
from zcls.model.backbones.resnetst_block import ResNetStBlock
...
def test_bottleneck():
    ...
    in_planes = 256
    out_planes = 128
    expansion = ResNetStBlock.expansion
    ...
    model = ResNetStBlock(in_planes, out_planes, stride, down_sample)
    print(model)
    ...
    model = ResNetStBlock(in_planes, out_planes, stride, down_sample, 32, 4)
    print(model)
...
def test_attention_bottleneck(attention_type='SqueezeAndExcitationBlock2D'):
    ...
    model = ResNetStBlock(in_planes=in_planes,
