...
def test_se_resnet_basicblock():
    ...
    inplanes = 64
    planes = 128
    expansion = AttentionResNetBasicBlock.expansion
    with_attention = True
    reduction = 16
    ...
    model = AttentionResNetBasicBlock(inplanes, planes, stride, downsample, with_attention=with_attention,
                                      reduction=reduction, attention_type=attention_type)
    print(model)

    outputs = model(data)
    print(outputs.shape)
    assert outputs.shape == (10, 128, 56, 56)
    ...
    outputs = model(data)
    print(outputs.shape)
    assert outputs.shape == (10, 128, 28, 28)
