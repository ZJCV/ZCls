...
class MNASNetBackbone(nn.Module, ABC):
    """
    参考Torchvision mnasnet实现
    在MnasNet-A1的Backbone之后添加了一个Conv1x1-BN-ReLU，最终输出特征维度为1280
    """
    ...
    def __init__(self,
        ...
        out_planes = 1280
        stage_setting = [
            # kernel_size, stride, expansion_rate, with_attention, repeated, out_planes
            [3, 1, 1, 0, 1, 16],
            [3, 2, 6, 0, 2, 24],
            [5, 2, 3, 1, 3, 40],
            [3, 2, 6, 0, 4, 80],
        ...
        for i, (kernel_size, stride, expansion_rate, with_attention, repeated, out_planes) in enumerate(stage_setting):
            features = list()
